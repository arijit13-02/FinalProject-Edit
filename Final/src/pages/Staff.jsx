...
function Staff() {
  ...
  const fetchData = async () => {
    try {
      const url = "http://192.168.0.110:5050/api/staff";
      if (!url) return;
    ...
    try {
      ...
      if (editingRecord) {
        ...
        response = await axios.put(
          `http://192.168.0.110:5050/api/staff/${editingRecord.id}`,
          form,
          { headers }
        );
        ...
        response = await axios.post(
          "http://192.168.0.110:5050/api/staff",
          form,
          { headers }
        );
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `http://192.168.0.110:5050/api/staff/${id}`,
        {
          headers: { "x-user-role": localStorage.getItem("userRole") }
        }
      );
  ...
  const handleExportXLS = async (record) => {
    try {
      ...
      if (record.attachment) {
        const fileUrl = `http://192.168.0.110:5050/Staffuploads/${record.attachment}`;
                ...
                <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
                  ...
                  <div className="w-40 h-40 flex-shrink-0 rounded overflow-hidden border border-gray-200">
                    {viewingRecord.attachment && !viewingRecord.attachment.endsWith(".pdf") ? (
                      <img
                        src={`http://192.168.0.110:5050/Staffuploads/${viewingRecord.attachment}`}
                        alt="Staff"
