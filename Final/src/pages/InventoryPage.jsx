...
const [lowStockItems, setLowStockItems] = useState([]);
  ...
  const fetchPendingChanges = async () => {
    try {
      const res = await axios.get(
        "http://172.20.10.11:5050/api/inventory/pending"
      );
  ...
  const loadRecords = async () => {
    try {
      const res = await axios.get("http://172.20.10.11:5050/api/inventory", {
        params: { role }
      });
      setRecords(res.data);
    } catch (error) {
      ...
      for (const row of importedData) {
        ...
        try {
          const response = await axios.post(
            `http://172.20.10.11:5050/api/inventory?role=${role}`, // role: 'admin' or 'staff'
            newRec,
            {
    ...
    if (editingRecord) {
      try {
        const response = await axios.put(
          `http://172.20.10.11:5050/api/inventory/${editingRecord.id}?role=${role}`, // update by ID
          {
            ...formData,
            id: editingRecord.id,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `http://172.20.10.11:5050/api/inventory/${id}?role=${role}`
      );
      loadRecords();
    } catch (err) {
