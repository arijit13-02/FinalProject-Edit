...
function UpcomingJobs() {
  ...
  const fetchData = async () => {
    try {
      const url = "http://192.168.0.105:5050/api/upcomingjobs";
      if (!url) return;
    ...
    if (editingRecord) {
      try {
        const response = await axios.put(
          `http://192.168.0.105:5050/api/upcomingjobs/${editingRecord.id}`, // update by ID
          {
            ...formData,
            id: editingRecord.id,
      ...
      try {
        ...
        const response = await axios.post(
          "http://192.168.0.105:5050/api/upcomingjobs",
          formData,
          {
            headers: {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `http://192.168.0.105:5050/api/upcomingjobs/${id}`,
        {
          headers: { "x-user-role": localStorage.getItem("userRole") }
        }
      );
