...
const Login = () => {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const res = await axios.post(
        "http://192.168.0.107:5050/api/change-password",
    ...
    try {
      const res = await axios.post(
        "http://192.168.0.107:5050/api/change-password1",
        { oldPassword: oldPwd, newPassword: newPwd },
        { withCredentials: true }
      );
  ...
  const handleLogin = async () => {
    if (role === "admin") {
      try {
        const res = await axios.post(
          "http://192.168.0.107:5050/api/login",
          {
            role,
            password
          },
      ...
      try {
        const res = await axios.post(
          "http://192.168.0.107:5050/api/login1",
          {
            role,
            password
          },
